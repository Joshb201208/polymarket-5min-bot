"""
Agent 2 — Soccer market analyzer.

Runs soccer-specific research: team form, injuries, league context.
"""
...
from agents.common.research_engine import ResearchResult, research_soccer_market

logger = logging.getLogger(__name__)


def analyze_market(market: dict, event: dict) -> dict | None:
    """Analyze a soccer market for edge. Returns alert dict or None."""
    ...
    logger.info("Analyzing soccer: %s (price=%.2f)", question[:60], market_prob)

    try:
        result: ResearchResult = research_soccer_market(market, event)
    except Exception as exc:
        logger.error("Soccer research failed for %s: %s", question[:60], exc)
        return None
    ...
    logger.info(
        "SOCCER EDGE: %s — edge=%.1f%% direction=%s",
