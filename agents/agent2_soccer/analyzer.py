"""Agent 2 Analyzer — soccer-specific research + fair probability calculation."""
...
def analyze_soccer_market(market: dict) -> dict | None:
    """Analyze a soccer market. Returns analysis dict if edge found, else None."""
    ...
    logger.info("Analyzing soccer: %s (YES @ %.2f)", question[:60], yes_price)

    # Step 1: Soccer-specific research
    search_query = _extract_soccer_query(question, description, event)
    research = research_soccer(search_query)
    ...
    if abs(edge) < MIN_EDGE_THRESHOLD:
        logger.debug("No edge for soccer market %s (edge=%.3f)", slug[:40], edge)
        return None
...
def _extract_soccer_query(question: str, description: str, event: dict) -> str:
    """Build a soccer-focused search query from market data."""
    event_title = event.get("title", "")
    # Prefer event title for broader context, fall back to question
