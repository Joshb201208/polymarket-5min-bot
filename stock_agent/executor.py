...
class Executor:
    ...
    async def place_buy(self, symbol: str, qty: int, stop_loss_price: float) -> dict | None:
        """Place a bracket order: market buy with attached stop-loss.

        POST /v2/orders with order_class=bracket.
        """
        if qty <= 0:
        ...
        logger.info("Placing BUY order: %s x%d with stop-loss $%.2f", symbol, qty, stop_loss_price)
        result = await self._request("POST", "/v2/orders", order_body)

        if result:
            logger.info("BUY order placed: %s — order_id=%s", symbol, result.get("id", "unknown"))
        else:
